...
import GraphViewer from "./components/features/GraphViewer";
...
function App() {
  ...
  const {
    ...
    reload: reloadGraph,
  } = useGraphData(DEFAULT_GRAPH_URL);
  const {
    papers,
    loading: papersLoading,
    deletePaper,
  } = usePaperData(DEFAULT_DB_PATH);
  ...
  const handleSelectPaper = useCallback((id: number) => {
    // Attempt to match paper ID to graph node ID if possible, otherwise just set it
    // Assuming paper.id might match node.id or node.paper_id
    // For now, simple set (graph might need a robust lookup if IDs differ)
